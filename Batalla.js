...
let quantityShip = [1, 2, 3, 4];
let quantityShipPC =  [1, 2, 3, 4];
let ship = {};
let shipRandom = {};
...
function checkShot(event){
    ...
    if(matrixAttack[x][y] === "ship"){
        ...
        alert("¡Disparo fallido! Intentalo de nuevo");
...
function shotPc(){
    ...
    if(matrix[x][y] === "ship"){
        alert("¡Cuidado acertaron a tu nave!");
        matrix[x][y] = "hit";
        ...
        alert("El jugador no dio en el blanco");
        matrix[x][y] = "miss";
...
function checkWinner(matrix, player){
    ...
    if(player === "pc"){
        alert("EL CONTRINCANTE HA GANADO!!!")
    }
    else{
